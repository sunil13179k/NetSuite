 ...
 ** @Description: Library script to create/update WooCommerce Products. out
 ** @NApiVersion 2.x
 ** @NModuleScope Public
 **/
...
define(['N/record', 'N/search', 'N/format', 'N/https', 'N/file'],
	function (record, search, format, https, file) {
			...
			try {
				...
				{ //Conditions......
					{ //Record Types
						...
						var matrixType = itemRecObj.getValue({
							fieldId: 'matrixtype'
						});
						log.debug({
					...
					if (item_type != 'itemgroup' && type != 'grouped') {
						{ //Get Price Information
							...
							if (salesPrice) {
								...
								updatesale = checksalesdate(salesStartDate, salesEndDate)
								log.audit({
						...
						{ //Get Inventory Details
							...
							if (manageStock == false) {
								...
								var preOrderEndDate = itemRecObj.getValue({
				...
				{ //attributes Details
					...
					if (nsAttArray.length > 0) {
						...
						var tagReturnObj = tagCatAttrObj(nsAttrObj, fieldsArry, matrixType);
						log.debug(itemRecId + ': attributes Object', 'Single attributes Obj: ' + JSON.stringify(tagReturnObj));
						if (tagReturnObj) {
							obj.attributes = tagReturnObj;
							log.debug({
				...
				{ //Images Details
					...
					var imageUpload = itemRecObj.getValue({
						fieldId: 'custitem_ns_woo_images_export'
					});
					...
					if (imageUpload == true && sr_ImageExport) {
						...
						var img1 = itemRecObj.getValue({
							fieldId: 'custitem_ns_woo_item_image1'
						});
						var img2 = itemRecObj.getValue({
							fieldId: 'custitem_ns_woo_item_image2'
						});
						var img3 = itemRecObj.getValue({
							fieldId: 'custitem_ns_woo_item_image3'
						});
						var img4 = itemRecObj.getValue({
							fieldId: 'custitem_ns_woo_item_image4'
						});
						if (img1)
							imageUrls.push(img1);
						if (img2)
						...
						if (imageUrls.length > 0) {
							for (var imgIndex = 0; imageUrls.length > imgIndex; imgIndex++) {
								var imgObj = {};
								imgObj.src = imageUrls[imgIndex].toString();
								imagesArray.push(imgObj);
								if (type == 'variation')
									singleImgObj = imgObj;
							}
						} else
							log.debug(itemRecId + ': Image Object details', ' Image URLs Does Not Exist In The Item Record.');
						if (type == 'variation')
							obj.image = singleImgObj;
						else
						...
						if (Number(responseCode) == 200 || Number(responseCode) == 201) {
							...
							if (updateProducts) {
								...
								if (responseBody.images.length > 0) {
									//var submitImgObj = {};
									for (imgInd = 0; imgInd < responseBody.images.length; imgInd++) {
										var resImgUrl = responseBody.images[imgInd];
										if (imgInd == 0) {
											childRecValues.custitem_woocom_image1 = resImgUrl.src.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, '');
										}
										if (imgInd == 1) {
											childRecValues.custitem_woocom_image2 = resImgUrl.src.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, '');
										}
										if (imgInd == 2) {
											childRecValues.custitem_woocom_image3 = resImgUrl.src.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, '');
										}
										if (imgInd == 3) {
											childRecValues.custitem_woocom_image4 = resImgUrl.src.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, '');
										}
									}
								}
							}
							if (matrixType == 'CHILD') {
								...
								if (matrixType == 'PARENT') {
									var attrFields = itemRecObj.getText({
										fieldId: 'matrixitemnametemplate'
									});
									var nsAttrArray = attrFields.slice(1, attrFields.length - 1).split('}-{');
									var matrixTempValue = attrFields.split(/[{}]/);
									log.debug({
										title: 'Variation Primary Data',
										details: 'nsAttrArray: ' + nsAttrArray
									});
									log.debug({
										title: 'Variation Primary Data',
										details: 'matrixTempValue: ' + matrixTempValue
									});
									log.debug('Matrix Item Details', ' Attributes Array Length:' + nsAttrArray.length);
										...
										for (var j = 1; j <= nsAttrArray.length; j++) { //Attributes length
											...
											for (var k in nsAttrArray) {
												...
												if (nsAttrArray[k].options) {
													log.debug('options index', (nsAttrArray[k].options).indexOf(lineItemTerm))
		...
		function tagCatAttrObj(nsRecArray, fieldsArry, matrixType) {
			var listLength = '';
			...
			if (listLength > 0) {
				...
				for (var recIndex in nsRecArray) {
					...
					log.debug({
						title: 'Number(nsRecArray[recIndex]): ',
						details: Number(nsRecArray[recIndex])
					})
					...
					var tagSearch_Result = tagSearchObj.run();
					var tagSearchResult = tagSearch_Result.getRange({
						start: 0,
						end: 1
					})[0];
					...
					if (tagSearchResult) {
						...
						if (fieldsArry[0] == 'attributes') {
							if (matrixType == 'PARENT') {
								var matrixTempValue = itemRecObj.getValue({
									fieldId: 'matrixitemnametemplate'
								});
								log.debug({
									title: 'Parent Matix Item',
									details: 'Matrix Template Name: ' + matrixTempValue
								});
								var matrixTempValue = matrixTempValue.split(/[{}]/);
								if (matrixTempValue) {
									for (var tempIndex in matrixTempValue) {
										if (matrixTempValue[tempIndex].length > 2 && matrixTempValue[tempIndex] != 'itemid') {
											var attObj = {};
											var fieldObj = itemRecObj.getField({
												fieldId: matrixTempValue[tempIndex]
											});
											log.debug({
												title: 'Attribute',
												details: fieldObj
											...
											var attributeSearch = search.create({
												...
												filters: [
													['custrecord_ns_woo_filed_id', 'is', matrixTempValue[tempIndex]],
													'and',
													['custrecord_ns_woo_mapping_filed', 'anyof', 1], //setupRecId
													'and',
													['isinactive', 'is', 'F']
												],
												columns: [
													search.createColumn({
														name: 'custrecord_ns_woo_attribute',
														label: 'WooCommerce attribute Name'
													}),
													search.createColumn({
											...
											var attributeSearch_Result = attributeSearch.run();
											var attFirstResult = attributeSearch_Result.getRange({
												start: 0,
												end: 1
											})[0];
											if (attFirstResult) {
												var wooAttId = attFirstResult.getValue(attributeSearch_Result.columns[1]);
												log.audit('Attribute Search', 'Woo Attribute Id: ' + wooAttId);

												attObj.id = wooAttId;
												var nsTermsArray = itemRecObj.getText({
													fieldId: matrixTempValue[tempIndex]
												});
												log.debug({
													title: 'Terms',
													details: nsTermsArray
												});
												attObj.options = nsTermsArray
												attObj.visible = true;

												if (nsTermsArray)
													attObj.variation = true;
											}
											/*  var fieldObj = itemRecObj.getField({ fieldId: matrixTempValue[tempIndex] });
											log.debug({ title: 'Attribute', details: fieldObj.label });
											var nsTermsArray = itemRecObj.getText({ fieldId: matrixTempValue[tempIndex] });
											log.debug({ title: 'Terms', details: nsTermsArray });
											if( wooRecName == fieldObj.label ){
												recObj.position	= 0;
												recObj.visible 	= true;
												if(nsTermsArray)
													recObj.variation = true;
												recObj.options 		= nsTermsArray;
											}*/
											wooRecArray.push(attObj);
										}
									}
								}
								...
								var lookupObj = search.lookupFields({
									type: 'customrecord_ns_woo_attributes_mapping',
									id: wooRecId,
									columns: ['custrecord_ns_woo_filed_id', 'custrecord_ns_woo_attribute']
								});
								log.debug({
									title: 'lookup Attribute Obj ',
									details: 'AttOption : ' + JSON.stringify(lookupObj)
								});
								var fieldID = lookupObj.custrecord_ns_woo_filed_id;
								var attributeName = lookupObj.custrecord_ns_woo_attribute[0].text;
								log.debug({
									title: 'Child Attribute Obj ',
									details: 'name : ' + attributeName
								});

								var colour = itemRecObj.getText({
									fieldId: fieldID
								});
								//var colour = itemRecObj.getValue({fieldId: fieldID});
								tagObj.name = attributeName;
								tagObj.option = colour[0];
								log.debug({
									title: 'Child Attribute Obj ',
									details: 'name : ' + attributeName + 'option: ' + colour + 'Colour Type: ' + typeof (colour)
								});
								log.debug({
									title: 'Child Attribute Obj ',
									details: 'AttOption : ' + JSON.stringify(tagObj)
								});
								...
								log.debug({
									title: 'term Name: ',
									details: attrObj[nsRecArray[recIndex]]
								})
								tagObj.position = 0;
								tagObj.visible = true;
		...
		function uploadImgToWP(itemImageFileIds, itemImageNames) {
			log.debug({
				title: 'Upload Images Function',
				details: 'upload images function Bigins'
			});
			var imgId = 0;
			var updateImgArry = [];
			for (var fileIndex = 0; itemImageFileIds.length - 1 > fileIndex; fileIndex++) {
				//if(itemImageFileIds[fileIndex])
				var imageObj = {};
				var imgObj = file.load({
					id: itemImageFileIds[fileIndex]
				});
				var imageBase = imgObj.getContents();
				var imageName = itemImageNames[fileIndex];
				...
				if (firstImgResult) {
					imgId = firstImgResult.getValue({
						name: "custrecord_ns_woo_image_id"
					});
					if (imgId) {
						imageObj.id = imgId;
						imageObj.name = imageName;
						imageObj.alt = imageName;
					}
				} else {
					var callLimit = 4;
					var uploadSuccess = false;
					var skip = false; //Skip response processing incase of error
					var headers = [];
					headers['Content-Type'] = 'image/' + imageName.split('.')[1].toLowerCase();
					headers['Filename'] = imageName.split('.')[0];
					var imgEndpoint = apiUrl.split("wc/v3/")[0] + "api/v1/upload_image";
					log.debug({
						title: 'Upload Imgs to Function',
						details: 'Image End Point: ' + imgEndpoint
					});
					do {
						callLimit--;
						log.debug({
							title: 'Upload Imgs to Function',
							details: 'Calls left: ' + callLimit
						});
						try {
							//return false;
							...
							log.debug({
								title: 'Upload Image Fn: POST: Catch Block',
								details: e
							});
							skip = true;
						}
						if (!skip) {
							...
							uploadSuccess = responseBody.success;
							log.debug({
								title: 'Upload Imgs to Function',
								details: 'Image upload status: ' + responseBody.success
							});
							if (uploadSuccess) {
								imgId = responseBody.id;
								log.debug({
									title: 'Upload Imgs to Function',
									details: 'Image Id: ' + imgId
								});
								...
								imgRecObj.setValue({
									fieldId: 'custrecord_ns_woo_image_id',
									value: imgId
								});
								imgRecObj.setValue({
								...
								log.debug({
									title: 'Upload Imgs to Function',
									details: 'Image Record Created with Id: ' + imgRecId
								})
								if (imgId) {
									imageObj.id = imgId;
									imageObj.name = imageName;
					...
					} while (callLimit > 0 && !uploadSuccess);
				} //else//if(firstImgResult)
				updateImgArry.push(imageObj);
			} //for( var fileIndex = 0; itemImageFileIds.length > fileIndex; fileIndex++ )
			log.debug({
				title: 'Upload Images Function',
				details: 'upload images function ends'
			});
