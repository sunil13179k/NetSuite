...
function( record, search, file, LIB_WooCommerce_Create_Customer, LIB_WooCommerce_Create_Order, LIB_WooCommerce_Create_Error_Record,LIB_WooCommerce_Create_Contact )
{
	function processOrder( order_details_json, orderDetails, stat )
	{
		try
		{
			...
			var dt = new Date();
			var singleOrderFile 	= file.create({ name: 'WooCommerce_Order_' + orderDetails['order_id'] + '_' + dt + '.json', contents: JSON.stringify(order_details_json), folder: orderDetails['orderFolder'], fileType: 'JSON' }); //contents: order_body_item,
			...
			//Stripe Charge Item
			{
				if(orderDetails['stripeChargeCheckBox'])
				{
					...
					log.audit({title: 'Stripe Charge Item Object',details:'Stripe Charge '+JSON.stringify(stripeObj)+' i:' + i} );
					}
					else
					{
						if(!orderDetails['stripeChargeKey'])
							log.audit({title: 'Stripe Charge Item',details:'Stripe Charge Key Not Set In the Setup reccord, '+ ' i:' + i });
						if(!orderDetails['stripeChargeItemId'])
							log.audit({title: 'Stripe Charge Item',details:'Stripe Charge Item Not Set In the Setup reccord, '+ ' i:' + i});
					}
				}
				else
				{
					log.audit({title: 'Stripe Charge Item',details:'Stripe Charge Check Box Is Unchecked in the Setup Record, '+ ' i:' + i});
				}
			}
				...
				for(var lineItemIndex = 0; lineItemIndex < order_item.length; lineItemIndex++)
				{
					...
					if(sku != null && sku != '')
					{
						...
						try
						{
							...
							if(!skipResults)
							{
								var item_Result 	= itemSearch.run().getRange({ start: 0, end: 10 });
			...
			else
			{
				...
				try
				{
					...
					else
					{
						...
						log.debug({ title: 'Customer Record Search', details: customerId+ ' i:' + i });
					}
				}//try
				catch (e)
				{
					customerId = 'Error in customer search. \nError: ' + e.name + '\nDetails: ' + e.message;
					log.debug({ title: 'Customer Search Catch Block Error', details: '\nError: ' + e.name + '\nDetails: ' + e.message });
				}
			}
